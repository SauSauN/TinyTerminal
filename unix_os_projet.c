...
void create_symbolic_link(Filesystem *fs, const char *link_name, const char *target_path) {
    // Vérifier si le système de fichiers a atteint sa capacité maximale
    if (fs->inode_count >= MAX_FILES) {
        printf("Le système de fichiers a atteint sa capacité maximale.\n");
        return;
    }

    // Construire le chemin absolu du fichier cible
    char absolute_target[MAX_PATH_LENGTH];
    snprintf(absolute_target, MAX_PATH_LENGTH, "/home/%s", target_path);

    // Vérifier si le fichier cible existe
    int target_exists = 0;
    for (int i = 0; i < fs->inode_count; i++) {
        if (strcmp(fs->inodes[i].name, absolute_target) == 0) {
            target_exists = 1; // Marquer que le fichier cible existe
            break; // Sortir de la boucle car le fichier cible a été trouvé
        }
    }

    // Si le fichier cible n'existe pas, afficher une erreur et sortir
    if (!target_exists) {
        printf("Erreur : le fichier cible '%s' n'existe pas.\n", target_path);
        return;
    }

    // Créer un nouvel inode pour le lien symbolique
    Inode *new_inode = &fs->inodes[fs->inode_count];

    // Initialiser les champs de l'inode
    strcpy(new_inode->name, link_name); // Copier le nom du lien symbolique
    new_inode->size = 0; // La taille d'un lien symbolique est généralement 0
    new_inode->is_directory = 0; // Un lien symbolique n'est pas un répertoire
    strncpy(new_inode->permissions, "lrwxrwxrwx", 11); // Définir les permissions du lien symbolique

    // Copier le chemin absolu du fichier cible dans le champ de lien symbolique
    strncpy(new_inode->lien.symbolicLink[0].data, absolute_target, sizeof(new_inode->lien.symbolicLink[0].data) - 1);
    new_inode->lien.symbolicLink[0].data[sizeof(new_inode->lien.symbolicLink[0].data) - 1] = '\0'; // Assurer la terminaison de la chaîne

    // Incrémenter le nombre d'inodes utilisés
    fs->inode_count++;

    // Sauvegarder le système de fichiers (si nécessaire)
    save_filesystem(fs);

    // Afficher un message de confirmation
    printf("Lien symbolique '%s' créé, pointant vers '%s'.\n", link_name, absolute_target);
}


// Fonction pour résoudre un lien symbolique et obtenir le chemin cible
char* resolve_symbolic_link(Filesystem *fs, const char *link_name) {
    // Parcourir tous les inodes dans le système de fichiers
    for (int i = 0; i < fs->inode_count; i++) {
        // Vérifier si l'inode actuel est un fichier (pas un répertoire) et un lien symbolique ('l' au début des permissions)
        if (fs->inodes[i].is_directory == 0 && fs->inodes[i].permissions[0] == 'l') {
            // Vérifier si le nom de l'inode correspond au nom du lien symbolique recherché
            if (strcmp(fs->inodes[i].name, link_name) == 0) {
                // Le lien symbolique a été trouvé

                // Créer une copie du chemin cible stocké dans l'inode
                // strdup alloue de la mémoire et copie la chaîne, ce qui est important car nous allons retourner cette chaîne
                char *target_path = strdup(fs->inodes[i].lien.symbolicLink[0].data);

                // Retourner le chemin cible copié
                return target_path;
            }
        }
    }

    // Si aucun lien symbolique correspondant n'est trouvé, retourner NULL
    return NULL;
}

// Fonction pour lire le contenu d'un fichier
void read_file(Filesystem *fs, const char *filename) {
    char *target_path = resolve_symbolic_link(fs, filename);
    if (target_path != NULL) {
        // Le fichier est un lien symbolique, utiliser le chemin cible
        read_file(fs, target_path + 6); // +6 pour enlever le /home/
        free(target_path); // Libérer la mémoire allouée par strdup
        return;
    }
...
void shell(Filesystem *fs, char *current_own) {
    ...
    while (current_own) {
        ...
        if (strncmp(command, "exit", 4) == 0) {
            ...
            // Extraire le nom du lien et la cible du lien
            sscanf(command + 3, "%s %s", link_name, target_path);
            create_symbolic_link(fs, link_name, target_path);
        } else if (strncmp(command, "resolv", 6) == 0) {
            char link_name[MAX_FILENAME];

            // Extraire le nom du lien symbolique à résoudre
            sscanf(command + 7, "%s", link_name);
            resolve_symbolic_link(fs, link_name);
        } else {
